...
async def get_summary(
    ...
    super_admins = (
        await db.scalar(
            select(func.count(UserRole.user_id.distinct()))
            .join(Role, Role.id == UserRole.role_id)
            .where(Role.role_name == ROLE_SUPER_ADMIN)
        )
        or 0
    )
    ...
    pending_blogs = (
        await db.scalar(select(func.count(Blog.id)).where(Blog.status == BLOG_STATUS_PENDING_REVIEW)) or 0
    )
    draft_blogs = await db.scalar(select(func.count(Blog.id)).where(Blog.status == BLOG_STATUS_DRAFT)) or 0
    rejected_blogs = (
        await db.scalar(select(func.count(Blog.id)).where(Blog.status == BLOG_STATUS_REJECTED)) or 0
    )
    ...
    logins_today = (
        await db.scalar(
            select(func.count(AuditLog.id)).where(
                AuditLog.action_type == ACTION_LOGIN, cast(AuditLog.created_at, Date) == today
            )
        )
        or 0
    )
    logouts_today = (
        await db.scalar(
            select(func.count(AuditLog.id)).where(
                AuditLog.action_type == ACTION_LOGOUT, cast(AuditLog.created_at, Date) == today
            )
        )
        or 0
    )
    failed_logins = (
        await db.scalar(
            select(func.count(AuditLog.id)).where(
                AuditLog.action_type == ACTION_FAILED_LOGIN, cast(AuditLog.created_at, Date) == today
            )
        )
        or 0
    )
    ...
    user_actions_today = (
        await db.scalar(
            select(func.count(AuditLog.id)).where(
                AuditLog.module == MODULE_USER_MANAGEMENT, cast(AuditLog.created_at, Date) == today
            )
        )
        or 0
    )
    role_changes_today = (
        await db.scalar(
            select(func.count(AuditLog.id)).where(
                AuditLog.module == MODULE_ROLE_MANAGEMENT, cast(AuditLog.created_at, Date) == today
            )
        )
        or 0
    )
...
async def get_recent(
    ...
    result = await db.execute(select(AuditLog).order_by(desc(AuditLog.created_at)).limit(20))
    return [
        {
...
async def get_charts(
    ...
    for i in range(6, -1, -1):
        ...
        count = (
            await db.scalar(
                select(func.count(AuditLog.id)).where(
                    AuditLog.action_type == ACTION_CREATE_BLOG, cast(AuditLog.created_at, Date) == d
                )
            )
            or 0
        )
    ...
    role_dist_result = await db.execute(
        ...
        .where(~Role.role_name.like("User Custom %"), ~Role.role_name.like("Admin Custom %"))
        .group_by(Role.role_name)
        .order_by(func.count(UserRole.user_id.distinct()).desc())
    )
    role_distribution = [{"name": name, "count": count} for name, count in role_dist_result.all()]
