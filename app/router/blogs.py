...
SUBMIT_BLOG_FOR_REVIEW = "Submit Blog For Review"


ALLOWED_IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png", ".webp"}
ALLOWED_STATUSES = {"Draft", PENDING_REVIEW, "Approved", "Published", "Rejected"}
...
async def is_super_admin(db: DBSessionDep, user: User) -> bool:
    return "Super Admin" in await get_user_role_names(db, user.id)
...
async def can_edit_blog(db: DBSessionDep, user: User, blog: Blog) -> bool:
    if await user_has_permission(db, user.id, "edit_blog"):
        return True
    return blog.author_id == user.id and await user_has_permission(db, user.id, "edit_own_blog")
...
async def get_blog_or_404(db: DBSessionDep, blog_id: int) -> Blog:
    blog = await db.scalar(
        select(Blog).options(*BLOG_LOAD_OPTIONS).where(Blog.id == blog_id)
    )
    if blog is None:
...
def status_for_action(action: str) -> str:
    return {"submit": PENDING_REVIEW, "publish": "Published"}.get(action, "Draft")
...
async def list_categories(db: DBSessionDep) -> list[BlogCategoryOut]:
    ...
    return [
        BlogCategoryOut(id=c.id, name=c.name, description=c.description)
...
async def list_blogs(
    ...
    await require_blog_permission(db, current_user, "view_blog")
    query = (
        select(Blog)
        .options(*BLOG_LOAD_OPTIONS)
        .order_by(Blog.id.desc())
    )
    is_admin = await is_super_admin(db, current_user)
    can_review = await user_has_permission(db, current_user.id, "review_blog")
    ...
    if search:
        ...
        query = query.join(User, User.id == Blog.author_id).where(
            or_(Blog.title.ilike(like), Blog.tags.ilike(like),
...
async def create_blog(
    ...
    await require_blog_permission(db, current_user, "create_blog")
    target_status = status_for_action(payload.action)
    if target_status == "Published":
        await require_blog_permission(db, current_user, "publish_blog")
    elif target_status == PENDING_REVIEW:
        await require_blog_permission(db, current_user, "submit_for_review")
    else:
        await require_blog_permission(db, current_user, "save_draft")
    ...
    log_audit_event(
        db, current_user, "Create Blog", BLOG_MANAGEMENT,
        f"{actor} created blog \"{blog.title}\" (Blog ID: {blog.id})", request
    )
    if target_status == PENDING_REVIEW:
        log_audit_event(
            db, current_user, SUBMIT_BLOG_FOR_REVIEW, BLOG_MANAGEMENT,
            f"{actor} submitted blog \"{blog.title}\" for review (Blog ID: {blog.id})", request
        )
...
async def update_blog(
    ...
    actor = await get_actor_label(db, current_user)
    log_audit_event(
        db, current_user, "Update Blog", BLOG_MANAGEMENT,
        f"{actor} edited blog \"{blog.title}\" (Blog ID: {blog_id})", request
    )
    if target_status == PENDING_REVIEW and old_status != PENDING_REVIEW:
        log_audit_event(
            db, current_user, SUBMIT_BLOG_FOR_REVIEW, BLOG_MANAGEMENT,
            f"{actor} submitted blog \"{blog.title}\" for review (Blog ID: {blog_id})", request
        )
...
async def update_blog_status(
    ...
    await require_blog_permission(
        db, current_user,
        "publish_blog" if payload.status == "Published" else "review_blog"
    )
    ...
    action, description = action_map.get(
        payload.status,
        ("Change Blog Status", f"{actor} changed blog \"{blog.title}\" to {payload.status}")
    )
...
async def feature_blog(
    ...
    await require_blog_permission(db, current_user, "feature_blog")
    blog = await get_blog_or_404(db, blog_id)
...
async def delete_blog(
    ...
    if blog.author_id != current_user.id:
        await require_blog_permission(db, current_user, "delete_blog")
    elif not await user_has_permission(db, current_user.id, "delete_blog"):
    ...
    log_audit_event(
        db, current_user, "Delete Blog", BLOG_MANAGEMENT,
        f"{actor} deleted blog \"{title}\" created by {original_author} (Blog ID: {blog_id})", request
    )
    await db.commit()
...
async def upload_blog_image(
    ...
    await require_blog_permission(db, current_user, "upload_blog_image")
    _, ext = os.path.splitext(file.filename or "")
    ext = ext.lower()
    if ext not in ALLOWED_IMAGE_EXTENSIONS:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Only JPG, JPEG, PNG, and WEBP are allowed"
        )
    upload_dir = Path(os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..", "uploads", "blogs")
    ))
...
async def blog_analytics(
    ...
    await require_blog_permission(db, current_user, "view_blog_analytics")
    total = await db.scalar(select(func.count(Blog.id))) or 0
    published = await db.scalar(select(func.count(Blog.id)).where(Blog.status == "Published")) or 0
    pending = await db.scalar(select(func.count(Blog.id)).where(Blog.status == PENDING_REVIEW)) or 0
    rejected = await db.scalar(select(func.count(Blog.id)).where(Blog.status == "Rejected")) or 0
    draft = await db.scalar(select(func.count(Blog.id)).where(Blog.status == "Draft")) or 0
    ...
    recent = await db.execute(
        select(Blog).options(*BLOG_LOAD_OPTIONS).order_by(Blog.id.desc()).limit(5)
    )
